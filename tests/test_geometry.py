...
def normalize_polygon(polygon):
    """
    Normalize a polygon so that its exterior and interior rings start at the minimum coordinate.
    """
    ext = list(polygon.exterior.coords)
    min_pt = min(ext)
    min_index = ext.index(min_pt)
    norm_ext = ext[min_index:] + ext[:min_index]

    norm_ints = []
    for interior in polygon.interiors:
        int_coords = list(interior.coords)
        min_pt = min(int_coords)
        min_index = int_coords.index(min_pt)
        norm_ints.append(int_coords[min_index:] + int_coords[:min_index])

    return Polygon(shell=norm_ext, holes=norm_ints)


def compare_geojson(geojson1, geojson2):
    geom1 = shape(geojson1)
    geom2 = shape(geojson2)

    if geom1.geom_type != geom2.geom_type:
        return False

    if geom1.geom_type == "Polygon":
        norm_geom1 = normalize_polygon(geom1)
        norm_geom2 = normalize_polygon(geom2)
        return norm_geom1.equals(norm_geom2)

    elif geom1.geom_type == "MultiPolygon":
        # Using 'geoms' property to avoid ShapelyDeprecationWarning
        sorted1 = sorted([normalize_polygon(p) for p in geom1.geoms], key=lambda p: p.wkt)
        sorted2 = sorted([normalize_polygon(p) for p in geom2.geoms], key=lambda p: p.wkt)

        return all(p1.equals(p2) for p1, p2 in zip(sorted1, sorted2))
